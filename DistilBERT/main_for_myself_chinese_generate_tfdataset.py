...
if __name__ == "__main__":
    ...
    model_name = '20230720-V2-generate_bert_model_epoch_tfdataset' + str(epochs)
    ...
    # try:
    #     with open('./data/keyword/' + model_name + '_generate_dict.yml', 'w', encoding='utf-8') as f:
    ...
    df2.to_csv('./data/keyword/' + model_name + '_generate_dict.csv', encoding='utf8')  

    df2 = pd.read_csv('./data/keyword/' + model_name + '_generate_dict.csv', encoding='utf-8')
    ...
    with tf.device("CPU"):
        data = tf.data.Dataset.from_tensor_slices((train_data.content.values, train_labels))
        data = data.shuffle(buffer_size=10000).batch(16)
